...
def process_files():
    ...
    for x in range(6):
        ...
        time.sleep(1)
    pb1.destroy()
    Label(window, text="Success!", foreground='green').grid(row=5,
                                                            columnspan=3,
                                                            pady=10)
    for x in range(len(lst)):
        Label(window, text=f"{lst[x]}", foreground="blue").grid(row=x + 5,
                                                                columnspan=3,
                                                                pady=10)
...
def mentee_to_dict():
    ...
    mentee_names = [
        x.title() for x in list(mentee_feedback["Name"]) if isinstance(x, str)
    ]
    mentee_emails = list(mentee_feedback["Email"])
    mentee_dict = {
        mentee_emails[x]: mentee_names[x]
        for x in range(len(mentee_names))
    }
...
def mentor_to_dict():
    ...
    mentor_names = [
        x.title() for x in list(mentor_feedback["Name"]) if isinstance(x, str)
    ]
    mentor_emails = list(mentor_feedback["Email"])
    mentor_dict = {
        mentor_emails[x]: mentor_names[x]
        for x in range(len(mentor_names))
    }
